...
purpose — the durable record lives in git (`git log --first-parent`)
and `plan/done/` (ADR 0037). The queued work lives in `plan/todo/`.
...
- **Branch:** main, tracking `origin/main` (PRs #1 to #4 merged).
- **Active item:** plan 0040 (in-flight derived: retire the dashboard, claim by branch) — dispatched to an Opus 5 worker in an Orca child worktree; closes with a PR.
- **Blockers:** none.
- **Uncommitted work:** none.

## Last shipped

- **2026-09-07 — plan 0039, retire the worklog** (PR #4, merge 64e8af3):
  ADR 0037 → Implemented. Transitional: the completion changes landed
  as a follow-up commit on `main` because the branch was still
  defining the rule that they ride on the claim branch. From plan 0040
  the worker ships before marking its request ready.
...
1. **Per item:** accept the owning ADR (delegated), dispatch an Opus 5
   ...
   merge. ADRs 0034–0037 Implemented, 0038 Accepted; 0039–0043
...
3. **Ship the queue in order**, 0040 → 0051 (0036–0039 shipped): shape-by-field (0036,
