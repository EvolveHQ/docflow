...
| OpenCode 1.18.30 / Big Pickle | Fresh bootstrap/new-adr and a native Task rung 2 case pass target checks with preserved signed concurrent claims. | Reporting failures remain recorded. A later retry fixes final-block placement but incorrectly integrates another item after the environment failure; the unchanged checks reject it. A fresh complete-contract retry is pending. Workers use explicitly assigned Git worktrees. |
| pi 0.84.4 / local Qwen coding route | A fresh native-high normal wave passes initial published-claim metadata, acquisition-before-write, signed completion and fresh-clone gate checks. | Earlier blocked flow passed but omitted the initial branch metadata. Fresh attempts retain interruptions and a local-backend 503 failure; full high-thinking acceptance remains pending. Local signed transport only. |
