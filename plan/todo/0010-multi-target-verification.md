...
1. Install docflow per the documented path — for **Codex** confirm
   `codex plugin marketplace add EvolveHQ/docflow` + `codex plugin install
   docflow` resolves the plugin (the `.codex-plugin/` + `.agents/plugins/
   marketplace.json` from plan 0011); OpenCode via auto-discovery/symlink;
   Cowork via the Claude Code plugin.
