...
The local Windows editor launcher was found but `cursor --help` failed
with missing `resources/app/out/cli.js`; this guide does not claim a
working local CLI or authenticated editor session.

Use an existing Cursor editor session at the selected member root and attach
the bounded brief plus native instructions. Keep the parent workspace context
explicit when the member does not contain it. Native skill locations and
discovery must be checked in the chosen session.
[Agent Skills](https://cursor.com/docs/skills).

The separate Cursor Agent CLI documents `agent` for interactive use,
`agent --mode=ask` for exploration and `agent --resume="<chat-id>"`
for a named continuation. Check the installed Agent executable and help
before use; the broken editor `cursor` launcher does not establish Agent
availability. [CLI reference](https://cursor.com/docs/cli/overview).

A native subagent receives its own complete brief; parent context and role
labels do not establish claims or tool permission. No cloud handoff or
installation follows from a selected profile.
