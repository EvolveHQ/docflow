# Codex App workspace guide

Guide revision 1; installed `codex app --help` checked 2026-09-15.
Coordinator preflight observed Windows app package 26.908.4834.0.
CLI help and desktop presence do not prove a usable App session.
...
Check that the desktop app is already installed before the second command:
installed help says it opens an installer when missing. Use it only with
explicit app-launch authority, otherwise open the existing app normally.
Select the member project and provide the bounded brief and read list.

Local skills are discovered through `.agents/skills`; the official guide
supports repository/user scope and symlinked skill folders. Keep complete
workspace assets with copied skills and resolve the actual source.
[Skill discovery](https://learn.chatgpt.com/docs/build-skills).

A native worktree can omit the ignored parent workspace. Supply an approved
snapshot or accessible source paths and recheck authority through the
coordinator before dependent action. Native worktree isolation does not
establish shared ownership. [Worktrees](https://learn.chatgpt.com/docs/environments/git-worktrees).
Qualify this App workflow separately from Codex CLI. Record app version,
platform, mode, actual actor/thread and member revision in the receipt.
