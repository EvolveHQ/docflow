...
| 2 | **Record model** — where capability content lives: **capability-first** (capability records in the ADR catalogue), **two-shape** (capability + technology shapes), **decisions+specs** (pure decision ADRs + living `spec/<slug>.md` records), or **decisions-only** (pure decisions; capability content owned by an external system). | **Capability-first** stays the default. **Scale rule:** small repo, few long-lived capabilities → capability-first; product repo, many living requirements → **decisions+specs** (capability growth becomes an edit, not a lifecycle round-trip). | The `model:` field in `docflow.yml`. Which shape `adr/0000-template.md` carries (decision shape on the decision-led models). On decisions+specs: the Capability Specs conventions section, `spec/0000-template.md`, and an empty `spec/`. Re-runs never convert a model. |
...
| 7 | **Optional artefacts** — `domains/` grouping, `GLOSSARY.md`, `CONSTRAINTS.md`, `goals/`, technology-ADR template. | **`domains/`: enable when the project has distinct areas or you expect >20 ADRs** (navigation by area). `GLOSSARY.md` / `CONSTRAINTS.md` / technology template: defer — the convention skill creates the glossary on your first shared term and the constraints file on your first boundary (each boundary needs an accepted decision record). `goals/`: defer unless you want the repo's "why" recorded from day one — `brainstorm` writes goal files once the layer exists and offers to enable it when an approved goal arrives without it. | Whether those files / directories are created; the constraints and goals choices also land in the `docflow.yml` `layers` list. |
...
| `goals/G-template.md` | `templates/goal.md` | The per-goal template (front matter: id = filename stem, title, state, horizon, review-by; body: Statement, Measure). Only if Q7 chose the goals layer; goal files arrive via `brainstorm` on approval, each with an INDEX Goals row. `COVERAGE.md` is generated later, not scaffolded. |
...
| `/release` | The outward ritual: release gate first (verify + deterministic evals + **behavioural suite at the release commit**), then promotion (you name the winning candidate; a decision record on the winner; `main` fast-forwards; alternatives archived unmerged), then one confirmation per outward step — three-manifest bump, tag, push, npm publish (one-shot userconfig, credentials never stored), GitHub release with reviewed notes. Refuses unattended invocation at every autonomy level; a declined step ends the ritual with an exact partial report. |
