...
optionally `_agent/prompts/autonomous.md` — written only where a verify
gate and the `plan/todo/` queue it walks both exist). For existing repos it
...
| 8 | **Verify gate** — what command(s) decide a change is shippable. | *No default — project-specific.* | Whether `_agent/prompts/autonomous.md` is written. The prompt needs both halves of its loop: the skill **refuses** to write it without a real verify gate, and equally without the `plan/todo/` queue it walks (Q4a). The Step 5 line of the prompt is filled with your command. |
...
| `_agent/prompts/autonomous.md` | `templates/_agent-prompts-autonomous.md` | Only if Q8 confirmed a verify gate **and** Q4a kept the `plan/todo/` queue the prompt walks. |
...
5. **Prepare integration.** Per the Q4b integration model:
   fast-forward locally toward `main` and wait to push (direct-to-main),
   or open a draft PR (PR-based).
6. **Ship.** Direct-to-main: commit the `git mv
   plan/todo/0001-<slug>.md plan/done/<YYYY-MM-DD>-<slug>.md`, shipped
   footer naming the HEAD SHA, ADR `Accepted`→`Implemented` update, and
   regenerated `INDEX.md`, then push. PR-based: make those completion
   changes as the final branch commit before marking the PR ready, with
   the shipped footer naming the pull request, then wait for CI and
   merge. Git history and the moved file are the shipped record; there
...
| `/ship-item` | §5 steps 5–6: verify gate → completion commit (`git mv` todo→done with footer, ADR `Accepted`→`Implemented`, regen INDEX) → direct push or PR merge per Q4b. The most order-sensitive operation; let the skill do it. |
