...
| 7 | **Optional artefacts** — `domains/` grouping, `GLOSSARY.md`, `CONSTRAINTS.md`, technology-ADR template. | **`domains/`: enable when the project has distinct areas or you expect >20 ADRs** (navigation by area). `GLOSSARY.md` / `CONSTRAINTS.md` / technology template: defer — the convention skill creates the glossary on your first shared term and the constraints file on your first boundary (each boundary needs an accepted decision record). | Whether those files / directories are created; the constraints choice also lands in the `docflow.yml` `layers` list. |
