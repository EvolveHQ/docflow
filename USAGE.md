...
| 7 | **Optional artefacts** — `domains/` grouping, `GLOSSARY.md`, `CONSTRAINTS.md`, `GOALS.md`, technology-ADR template. | **`domains/`: enable when the project has distinct areas or you expect >20 ADRs** (navigation by area). `GLOSSARY.md` / `CONSTRAINTS.md` / technology template: defer — the convention skill creates the glossary on your first shared term and the constraints file on your first boundary (each boundary needs an accepted decision record). `GOALS.md`: defer unless you want the repo's "why" recorded from day one — `brainstorm` writes entries once the layer exists and offers to enable it when an approved goal arrives without it. | Whether those files / directories are created; the constraints and goals choices also land in the `docflow.yml` `layers` list. |
...
| `GOALS.md` | `templates/GOALS.md` | The repo's 3–7 active goals (`G-<slug>` entries: statement, measure, horizon, review-by, state). Only if Q7 chose the goals layer; entries arrive via `brainstorm` on approval. `COVERAGE.md` is generated later, not scaffolded. |
