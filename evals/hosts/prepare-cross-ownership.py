...
for s,root in roots.items():
 ...
 (root/'AGENTS.md').write_text('# Workspace instructions\nUse the installed workspace-dispatch skill and packaged assets. Canonical records are under .docflow_workspace; linked workspace and member roots are explicitly authorised for read-only inspection. TASK.md remains the sole native member task. Do not create a second native queue.\n')
